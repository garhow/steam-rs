...
use crate::{macros::{gen_args, do_http, optional_argument}, errors::{ErrorHandle, SteamEconomyError}, Steam};
...
impl Steam {
    ...
    pub async fn get_asset_class_info(&self, appid: u32, language: Option<&str>, class_count: u32, classid0: u64, instanceid0: Option<u32>) -> Result<HashMap<String, AssetClassInfo>, SteamEconomyError> {
        let key = &self.api_key.clone();
        let args = gen_args!(key, appid, class_count, classid0) + &optional_argument!(language, instanceid0);
        let url = format!("{END_POINT}{args}");
        let response = do_http!(url, UncleanAssetClassInfo, ErrorHandle, SteamEconomyError::GetAssetClassInfo).clean()?;
        Ok(response)
    }
}
...
impl UncleanAssetClassInfo {
    pub fn clean(mut self) -> Result<HashMap<String, AssetClassInfo>, SteamEconomyError> {
        ...
        let data = self.result.par_iter().map(move | (id, data) | -> (String, AssetClassInfo) {
            println!("{data:?}");
            (id.clone(), serde_json::from_value::<AssetClassInfo>(data.to_owned()).unwrap() )
        }).collect::<Vec<(String, AssetClassInfo)>>();

        // Convert the data into a hashmap
        let data = data.into_iter().collect::<HashMap<String, AssetClassInfo>>();
        Ok(data)
    }
}