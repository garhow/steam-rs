...
#[derive(Debug, Deserialize, Serialize)]
struct Wrapper { response: CommunityBadgeProgress }
...
impl Steam {
    pub async fn get_community_badge_progress(
        ...
        let query = format!("?key={}&steamid={}{}", &self.api_key, steam_id, optional_argument!(badge_id, "badgeid"));
        let url = format!("{}/{}/{}/v{}/{}", BASE, INTERFACE, ENDPOINT, VERSION, query);
        let json = do_http!(url, Value, ErrorHandle, PlayerServiceError::GetCommunityBadgeProgress);
        let community_badge_progress: Wrapper = ErrorHandle!(serde_json::from_value(json.to_owned()), PlayerServiceError::GetCommunityBadgeProgress);
