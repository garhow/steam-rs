...
pub async fn get_global_achievement_percentages_for_app() {
    ...
    assert!(steam
        .get_global_achievement_percentages_for_app(EXAMPLE_APP_ID).send()
        .await
        .is_ok());
}
...
pub async fn get_number_of_current_players() {
    ...
    // Expected result
    assert!(steam
        .get_number_of_current_players(EXAMPLE_APP_ID).send()
        .await
        .is_ok());

    // Error condition (nonexistent app)
    assert!(steam.get_number_of_current_players(1).send().await.is_err());
}
...
pub async fn get_player_achievements() {
    ...
    assert!(steam
        .get_player_achievements(EXAMPLE_STEAM_ID_PUBLIC, EXAMPLE_APP_ID).send()
        .await
        .is_ok());
    // Error condition (private profile)
    assert!(steam
        .get_player_achievements(EXAMPLE_STEAM_ID_PRIVATE, EXAMPLE_APP_ID).send()
        .await
        .is_err());
}
...
pub async fn get_schema_for_game() {
    ...
    assert!(steam
        .get_schema_for_game(EXAMPLE_APP_ID).send()
        .await
        .is_ok());
}
